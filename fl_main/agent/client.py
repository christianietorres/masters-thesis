...
class Client:
    ...
    def wait_for_global_model(self):
        """
        Blocks until a global model is available and transitions client state
        to 'training'. Loads and returns the model and its context.

        Returns:
            global_models (Any): Loaded global model(s).
            model_path (str): Path where the model was stored.
            gmfile (str): Filename of the global model.
            data_dict (Dict[str, Any]): Full dictionary containing models and metadata.
        """
        ...
        return global_models, self.model_path, self.gmfile, data_dict
