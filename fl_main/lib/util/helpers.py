...
def save_model_file(data_dict: Dict[str, Any],
                    path: str,
                    name: str,
                    performance_dict: Dict[str, Any] = dict(), 
                    final_flag: bool = False, 
                    FL_flag: bool = True):
    """
    Save a given set of models into a specific file, optionally saving them
    under a final model name depending on context.

    Args:
        data_dict (Dict[str, Any]): Dictionary containing model data 
        (e.g., model_id, models).

        path (str): Path to the directory where the file will be saved.

        name (str): Base name of the model file.

        performance_dict (Dict[str, Any], optional): each entry contains 
        a pair of a model id and its performance metrics (e.g., accuracy). 
        Defaults to empty dict.

        final_flag (bool, optional): If True, prepends filename 
        with a final model prefix. 
        Defaults to False.

        FL_flag (bool, optional): Used to distinguish between FL and CL models.
                                  If True, saves as "final_global_model{name}"; 
                                  else "final_CL_model{name}".
    ...
    if final_flag == True:
        if FL_flag == True:
            final_name = "final_global_model"
        else:
            final_name = "final_CL_model"
